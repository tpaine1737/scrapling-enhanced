"""Enhanced DynamicFetcher backed by Camoufox instead of Playwright Chromium."""
...
class DynamicFetcher(_OriginalDynamicFetcher):
    ...
    def fetch(cls, url: str, **kwargs: Any) -> Response:
        """Fetch *url* using a Camoufox-backed browser session.

        Accepts all keyword arguments that Scrapling's ``DynamicFetcher.fetch()``
        accepts — they are forwarded unchanged to
        :class:`~scrapling_enhanced.engine._base.CamoufoxDynamicSession`.
